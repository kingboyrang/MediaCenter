...
{
    CGFloat w=(BUTTONWIDTH+BUTTONGAP)*[self.TitleArray count]+BUTTONGAP;
     if (self = [super init])
     {
          ...
          YGPScrollView.contentSize = CGSizeMake(w, 40);
     ...
     //初始化button
     [self initWithButton];
    if (totalW<=Frame.size.width&&w<=totalW) {
        CGRect rect=YGPScrollView.frame;
        rect.size.width=totalW;
        rect.origin.x=(Frame.size.width-totalW)/2.0;
        [YGPScrollView setFrame:rect];
    ...
    int buttonPadding = 18;
     ...
     for (int i = 0; i<[self.TitleArray count]; i++)
     {
         ...
         strings = [NSString stringWithFormat:@"%f",SegmentedButton.frame.size.width];
          ...
          totalW+=SegmentedButton.frame.size.width;
...
-(void)segmentedSelectedIndex:(NSInteger)index{
